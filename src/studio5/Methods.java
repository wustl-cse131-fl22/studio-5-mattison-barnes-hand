...
public class Methods {
	...
	public static double distanceBetween(double x1, double y1, double x2, double y2) {
		...
		double xSqDifference = xDifference * xDifference; 
		double ySqDifference = yDifference * yDifference;
		distance = Math.sqrt(xSqDifference + ySqDifference);		
		return distance;
	}
	...
	public static void drawBullsEye(double x, double y, double radius) {
		...
		// Draw the remaining rings of the bull's eye
	...
	public static String substituteAll(String source, char target, String replacement) {
		...
		for (int count = 0; count < characters.length; count++) {
			if (characters[count] == target) {
				result = result + replacement;
			}
			else {
			result = result + characters[count];
			}
		}
		return result;
	}
